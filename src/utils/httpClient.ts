import type { APIError, ResultResponse } from '../api/type';
...
export class HttpClient {
  ...
  protected async request<T>(
    ...
    try {
      ...
      if (!response.ok) {
        const errorData: APIError = await (response.json() as Promise<APIError>).catch(
          () => ({
      ...
      const result = (await response.json()) as ResultResponse<T>;
